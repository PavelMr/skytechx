...
////////////////////////////////////////////////////////////////////////////////
{
  ...
  //int i= 0;
  {
    ...
    sat->sat[i].throwShadow = plshadow(o, sun, o->poleRa, o->poleDec, sat->sat[i].x, sat->sat[i].y, sat->sat[i].z, &sat->sat[i].sx, &sat->sat[i].sy, sat->sat[i].inSunLgt);

    //sat->sat[i].sx = 0.5;
    //sat->sat[i].sy = 0.5;

    if (pln == PT_JUPITER)
      //qDebug() << "o" << sat->sat[i].sx << sat->sat[i].sy << sat->sat[i].x << sat->sat[i].y << sat->sat[i].z;
 ...
 */
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool CSatXYZ::plshadow(orbit_t *op, orbit_t *sop, double polera, double poledec, double x, double y, double z, double *sxp, double *syp, bool &inLight)
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
{
  double flat = op->sx / op->sy;

  /* equatorial to ecliptic sky-plane rotation */
  double sa = cos(op->gRD.Dec) * cos(poledec) *
             (cos(op->gRD.Ra) * sin(polera) - sin(op->gRD.Ra) * cos(polera));
...
//////////////////////////////////////////////////////////
bool CSatXYZ::moonSVis(orbit_t *o, orbit_t *s, double x, double y, double z)
//////////////////////////////////////////////////////////
{
  double flat = o->sx / o->sy;
  double esd = s->R;
  ...
  double xp =  sca * x + ssa * z;
  double yp =  y;// * flat;
  double zp = -ssa * x + sca * z;
  double ca = cos(nod), sa = sin(nod);
  double xpp = xp;
  double ypp = ca * yp - sa * zp;
  double zpp = sa * yp + ca * zp;
