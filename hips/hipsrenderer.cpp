...
{  
  ...
  if (SKPLANECheckFrustumToPolygon(trfGetFrustum(), pts, 4))
  {
    ...
    HipsImage *image = m_manager.getPix(allsky, level, pix, freeImage);
    ...
    if (image)      
    {
      ...
      if (image->m_items.count() > 0)
      { // catalogue
        for (int i = 0; i < image->m_items.count(); i++)
        {
          SKPOINT pt;

          trfRaDecToPointNoCorrect(&image->m_items[i].rd, &pt);
          if (trfProjectPoint(&pt))
          {
            painter->setPen(Qt::red);
            painter->drawCircle(QPoint(pt.sx, pt.sy), 2);
          }
        }
      }
      else
